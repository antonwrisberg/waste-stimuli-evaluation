// Recognition
export default function (jsPsych, timeline, HtmlButtonResponsePlugin, detectLanguage, recognitionTrialCount, stimuliAllAsItem) {
  ...
  timeline.push({
    ...
    stimulus: function() {
      switch (detectLanguage()) {
        case 'en':
          return `
            <p>In a moment, you will see ${recognitionTrialCount} images of waste.</p>
            <p>For each image, you should decide whether you could explain to someone else what it is.</p>
          `;
        case 'da':
          return `
            <p>Om et øjeblik vil du se ${recognitionTrialCount} billeder af affald.</p>
            <p>For hvert billede, skal du afgøre, om du ville kunne foreklare en anden, hvad det er.</p>
          `;
        case 'sv':
          return `
            <p>Inom kort kommer du att se ${recognitionTrialCount} bilder på avfall.</p>
